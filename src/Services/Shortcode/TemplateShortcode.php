...
namespace Netauratech\ContentManager\Services\Shortcode;
...
class TemplateShortcode
{
     ...
     *
     * @return string Rendered HTML of the view `content-manager::shortcodes.template`,
     ...
     * // `content-manager::shortcodes.template` with $template = Content::find(3).
     */
    public function __invoke(array $attrs, array $context): string
    {
        ...
        return view('content-manager::shortcodes.template', [
                'template' => $template,
            ] + $context)->render();
    }
}